...
export class MapTravelService {
  ...
  toJSON(travelString) {
    ...
    const travelerZones = travelerStrings.map(traveler => {
      const travelerZone = /\.([0-9_]*)z/i.exec(traveler);
      return travelerZone ? this.makeJSONZone(travelerZone[1]) : [];
    });
    // {x: 50, y: 50} {x: 5, y: 20}
    ...
    */
    ...
    const travelers = zoneIndexes.map((zoneIndex, i) => {
      ...
      return { zoneIndex: 0, x, y, originalPositions: { x, y } };
    });
